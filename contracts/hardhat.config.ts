 ...
 * so the site can never drift from the contracts. SKIP_ABI_EXPORT=true opts out.
 */
...
const config: HardhatUserConfig = {
  ...
  networks: {
    hardhat: {
      ...
      // FORK_URL=https://rpc.mainnet.chain.robinhood.com forks Robinhood Chain in-process.
      ...(process.env.FORK_URL
        ? { forking: { url: process.env.FORK_URL, ...(process.env.FORK_BLOCK ? { blockNumber: Number(process.env.FORK_BLOCK) } : {}) } }
        : {}),
    },
    localhost: { url: "http://127.0.0.1:8961", timeout: 600_000 },
    robinhood: { url: ROBINHOOD_RPC_URL, chainId: ROBINHOOD_CHAIN_ID, accounts },
  },
  etherscan: {
